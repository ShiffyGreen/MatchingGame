...
// Constants and variables with type annotations
...
let firstSelection: Button | null = null;
let secondSelection: Button | null = null;
let lockBoard: boolean = false;
let matchedPairs: number = 0;
let currentPlayer: number = 1;
let player1Matches: number = 0;
let player2Matches: number = 0;
...
function setupGame(): void {
    ...
    if (turnLabel) turnLabel.style.display = 'block';
    player1Matches = 0;
    player2Matches = 0;
    updateMatchCounts();
}
...
function resetSelections(isMatch: boolean): void {
    firstSelection = null;
...
function updateTurnLabel(message?: string): void {
    const turnLabel = document.getElementById('turnLabel');
    if (!turnLabel) return;
...
function updateMatchCounts(): void {
    const player1Element = document.getElementById('player1Matches');
    const player2Element = document.getElementById('player2Matches');

    if (player1Element) player1Element.textContent = player1Matches.toString();
    if (player2Element) player2Element.textContent = player2Matches.toString();
}
